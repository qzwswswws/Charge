"""
P5-A1: multi-seed reproducibility for c3c4 / 2-class.

Strategy:
1. Reuse the existing seed=42 baseline rows from degradation_2class.csv.
2. Run only the missing seeds.
3. Write a raw CSV and a subject/overall summary CSV.

Usage:
    python run_multiseed_c3c4_2class.py
"""
...
def main() -> None:
    ...
    seed42_rows = load_seed42_baseline(summary_dir / "degradation_2class.csv")
    raw_rows: list[list[str]] = []

    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    raw_path = summary_dir / f"multi_seed_c3c4_2class_raw_{ts}.csv"
    summary_path = summary_dir / f"multi_seed_c3c4_2class_summary_{ts}.csv"
    latest_raw = summary_dir / "multi_seed_c3c4_2class_raw_latest.csv"
    latest_summary = summary_dir / "multi_seed_c3c4_2class_summary_latest.csv"
    total_start = datetime.datetime.now()

    print(f"{'=' * 72}")
    print("  P5-A1 multi-seed reproducibility")
    ...
    for seed in SEEDS:
        for subject in SUBJECTS:
            if seed == 42 and subject in seed42_rows:
                ...
                print(f">>> Reusing subject={subject}, seed=42 from degradation_2class.csv")
                continue
    ...
    print(f"\n{'=' * 72}")
    print(f"  P5-A1 finished. Total duration: {total_end - total_start}")
